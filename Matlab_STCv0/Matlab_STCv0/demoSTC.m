...
%% set path
addpath('./da/img');
img_dir = dir('./da/img/*.jpg');
%% initialization
initstate = [198,214,34,81];%initial rectangle [x,y,width, height]
...
for frame = 1:numel(img_dir),
    ...
    rect_position
    for i=1:length(rect_position)
        fprintf(fid,'%d',floor(rect_position(i)));
        if i <4
             fprintf(fid,',');
        end
    end
    ...
    imagesc(uint8(img))
    colormap(gray)
    rectangle('Position',rect_position,'LineWidth',2,'EdgeColor','b');
    hold on;
    text(5, 18, strcat('#',num2str(frame)), 'Color','y', 'FontWeight','bold', 'FontSize',15);
